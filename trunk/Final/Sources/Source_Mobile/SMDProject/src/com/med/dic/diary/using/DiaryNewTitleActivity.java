...
public class DiaryNewTitleActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		diaryStartBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				new DatePickerDialog(DiaryNewTitleActivity.this, date, myCalendar
		...
		diarySaveBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				String diaryTitle = ((EditText) findViewById(R.id.diaryTitleEdt)).getText().toString();
				...
				if(diaryTitle == null || diaryTitle.equals("")) {
					AlertDialog.Builder _builder = new AlertDialog.Builder(DiaryNewTitleActivity.this);
						...
						if ((!diaryStart.equals(DefaultValueConstant.DIARY_USING_DEFAULT_DATE) && diaryEnd.equals(DefaultValueConstant.DIARY_USING_DEFAULT_DATE)) ||
							...
							try {
								if (mCursor.getCount() == 0) {
									myDB.insertDiaryTitle(i, diaryTitle, diaryStart, diaryEnd);
									i++;
								} else {
									mCursor.moveToFirst();
									sequence = Integer.parseInt(mCursor.getString(0)) + 1;
									myDB.insertDiaryTitle(sequence, diaryTitle, diaryStart, diaryEnd);
								}
							} finally {
								mCursor.close();
								myDB.close();
							...
							for (int j = 0; j < dt.size(); j++) {
								...
								alarmintent.putExtra("Note", "Thuốc: " + dt.get(j).getMedicine() + "\n" + "Liều lượng: " + dt.get(j).getAmount());
							...
							Intent _intent = new Intent(DiaryNewTitleActivity.this, MedicationDiaryActivity.class);
		...
		diaryCancelBtn.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View arg0) {
				AlertDialog.Builder _builderConfirm = new AlertDialog.Builder(DiaryNewTitleActivity.this);
				...
				_builderConfirm.setPositiveButton("Đồng ý", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						if (diaryTitle != null) {
							myDB.open();
							myDB.removeDiaryMedicine(diaryTitle.getTitleId());
							myDB.close();
						}
						Intent _intent = new Intent(DiaryNewTitleActivity.this, MedicationDiaryActivity.class);
						startActivity(_intent);
		...
		diaryAddMedicineBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				diaryTitle = new DiaryTitle(0, title, startDate, endDate);
				diaryTitle.setMode("MODE_ADD");
				Intent _intent = new Intent(DiaryNewTitleActivity.this, DiaryNewMedicineActivity.class);
		...
		adapter = new DiaryMedicineAdapter(DiaryNewTitleActivity.this, android.R.layout.simple_list_item_1, myList);
		...
		diaryMedicineLv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				medicine.setMode("MODE_ADD");
				Intent _intent = new Intent(DiaryNewTitleActivity.this, DiaryViewMedicineActivity.class);
